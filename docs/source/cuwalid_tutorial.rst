...
- STORM config (`storm_input_ICPAC.json`)
...
- `NSIM` is the number of ensemble simulations for STORM, stoPET, and DRYP (optional).
- `NSIM_HYDRO` If you want to run DRYP ensemble with a different size, `NSIM` will be used as default (see below).
...
- bounding box (`latval_min/max`, `lonval_min/max`), default values are for the Horn of Africa region
- `locname`
- `tempAdj`, etc.
