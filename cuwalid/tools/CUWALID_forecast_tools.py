...
def plot_impact_tercile_forecast(data,
	...
	data_tercile = data[tercile[0]]
	data_tercile = np.where(data_tercile > 0.3, 1, np.nan)
	g1 = data_tercile.plot(
			x="lon", y="lat",
			vmin=0.0, vmax=1.0,
			cmap="spring",#plt.get_cmap(cmap[0], 5),
			ax=ax,
			add_colorbar=False,
			)
	
	data_tercile = data[tercile[1]]
	data_tercile = np.where(data_tercile > 0.3, 1, np.nan)
	g1 = data_tercile.plot(
			x="lon", y="lat",
			vmin=0.0, vmax=1.0,
			cmap="wistia",#plt.get_cmap(cmap[0], 5),
			ax=ax,
			add_colorbar=False,
			)

	data_tercile = data[tercile[2]]
	data_tercile = np.where(data_tercile > 0.3, 1, np.nan)
	g1 = data_tercile.plot(
			x="lon", y="lat",
			vmin=0.0, vmax=1.0,
			cmap="winter",#plt.get_cmap(cmap[0], 5),
			ax=ax,
			add_colorbar=False,
			)
