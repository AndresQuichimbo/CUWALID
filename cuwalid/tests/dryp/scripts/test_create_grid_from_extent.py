...
def test_create_grid_from_extent_projected():
    ...
    grid = create_grid_from_extent(ncols, nrows, lon_min, lat_min, cellsize, domain=None, geographic=True)
    ...
    expected_core = np.where(center_ones(nrows, ncols).flatten() > 0)[0]
