...
How to run the model:
	1. Create an input json file with the model parameters and settings. An example of the input json file can be found in the documentation.
	2. Run the model using the command line above, replacing -filename_input.json- with the path to your input json file.
	mpirun -n <N> python main_parDRYP_dynamic.py <config.json>
"""
...
def _dynamic_rank_assignment(task_ids, task_costs, size):
	...
	if size <= 1 or len(task_ids) == 0:
		return [list(task_ids)] if size > 0 else []

	costs = np.asarray(task_costs, dtype=float)
	if costs.size != len(task_ids):
		costs = np.ones(len(task_ids), dtype=float)
	costs = np.where(np.isfinite(costs) & (costs > 0.0), costs, 1.0)
	...
	for idx in order:
		...
		assignments[r].append(task_ids[int(idx)])
		loads[r] += costs[int(idx)]
...
def run_parDRYP(filename_input, disable_dynamic=False):
	...
	use_dynamic_scheduling = not disable_dynamic
	if use_dynamic_scheduling and runoff_parallel:
		basis_assignments = _dynamic_rank_assignment(basin_ids, basis_costs, size)
	else:
		basis_assignments = partools.assign_basins_to_rank(basin_ids, rank, size) if runoff_parallel else []
	local_basin_ids = basis_assignments[rank] if (runoff_parallel and use_dynamic_scheduling) else (basis_assignments if isinstance(basis_assignments, list) and len(basis_assignments) > rank else [])
	...
	if use_dynamic_scheduling and gw_parallel:
		gw_assignments = _dynamic_rank_assignment(gw_region_ids, gw_costs, size)
	else:
		gw_assignments = partools.assign_basins_to_rank(gw_region_ids, rank, size) if gw_parallel else []
	...
	def _ensure_runoff_basin_initialized(basin_id):
		if basin_id in basis_ro:
			return
	...
	def _ensure_gw_region_initialized(region_id):
		nonlocal gw_split_lakes_detected
		if region_id in region_gw:
			return
		...
		for UZ_ti in range(data_in.dt_hourly):
			...
			for dt_pre_sub in range(data_in.dt_sub_hourly):
				...
				if runoff_parallel:
					...
					if comm is not None:
						global_cost_sum = _allreduce_sum(comm, local_cost_sum)
						global_cost_count = _allreduce_sum(comm, local_cost_count)
						mask_cost = global_cost_count > 0.0
						if np.any(mask_cost):
							new_cost = global_cost_sum[mask_cost] / global_cost_count[mask_cost]
							basis_costs[mask_cost] = (1.0 - basis_cost_alpha)*basis_costs[mask_cost] + basis_cost_alpha*new_cost
				...
				if data_in.run_GW > 0:
					if dt_GW == data_in.dtSZ:
						...
						if gw_parallel:
							...
							if use_dynamic_scheduling:
								gw_assignments = _dynamic_rank_assignment(gw_region_ids, gw_costs, size)
								local_gw_region_ids = gw_assignments[rank]
								for region_id in local_gw_region_ids:
									_ensure_gw_region_initialized(region_id)
							...
							local_gw_cost_count = np.zeros(len(gw_region_ids), dtype=float)
							...
							for region_id in local_gw_region_ids:
								...
								idx = gw_id_to_idx.get(region_id)
								if idx is not None:
									local_gw_cost_sum[idx] += (time.time() - iregion_start)
									local_gw_cost_count[idx] += 1.0
								# print(C)
								# print(local_head)
							...
							mask_gw_cost = global_gw_cost_count > 0.0
							if np.any(mask_gw_cost):
								new_gw_cost = global_gw_cost_sum[mask_gw_cost] / global_gw_cost_count[mask_gw_cost]
								gw_costs[mask_gw_cost] = (1.0 - gw_cost_alpha)*gw_costs[mask_gw_cost] + gw_cost_alpha*new_gw_cost
...
if __name__ == '__main__':
	...
	parser.add_argument('--disable-dynamic', action='store_true', 
					help='Disable dynamic scheduling and use static round-robin assignment instead')

	# Parse command line arguments
	args = parser.parse_args()

	run_parDRYP(args.config_file, disable_dynamic=args.disable_dynamic)
# ---------------------------------------------------------------------