...
IMPLICIT NONE
CONTAINS
SUBROUTINE find_discharge_and_losses(s, r, Criv, Kt,riv, Q_aoft, riv_std,&
P3, P4, discharge, Qtl, Q_ini, Q_aof)
INTEGER, INTENT(IN) :: s(:)
INTEGER, INTENT(IN) :: r(:)
...
P3(donor), P4(donor), Q_out, Q_TLp, Q_inip, Q_aofp)
...
SUBROUTINE TRANSLOSS(Criv, Kt, riv, Qaoft, riv_std, P3, P4, Qout, QTL, Q_ini, Qaof)
REAL, INTENT(IN) :: Criv, Kt, Qaoft, riv_std, P3, P4
INTEGER, INTENT(IN) :: riv
...
!CALL FUNCTION FOR NO TRANSMISSION LOSSES
CALL EXP_DECAY(Qin, Kt, Qout, Qo)
TL = 0
ELSE
!CALL FUNCTION FOR TRANSMISSION LOSSES
CALL EXP_DECAY_LOSS(Criv, Qin, Kt, P3, P4, Qout, TL, Qo)

!CHECK IF TRANSMISSION LOSSES ARE HIGHER THAN STORAGE AVAILABLE
...
!compare if transmission losses is greater that
!river cell and riparian moisture deficit
